...
public class OrganizationControllerTest extends RestIntegrationTest {
    ...
    public void should_create_organization() {
        ...
        assertThat(organizationRepository.findById(responseEntity.getBody().getId())).isPresent();
    }

    @Test
    public void should_get_by_id_organization() {
        //OPERATE
        ResponseEntity<OrganizationDTO> responseEntity = restTemplate.getForEntity(OrganizationEndPoints.ORGANIZATION_RESOURCE_ID , OrganizationDTO.class, organization.getId());

        //CHECK
        assertThat(responseEntity.getStatusCodeValue()).isEqualTo(200);
        assertThat(responseEntity.getBody().getId()).isEqualTo(organization.getId());
    }
}